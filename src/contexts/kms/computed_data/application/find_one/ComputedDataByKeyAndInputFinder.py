...
class ComputedDataByKeyAndInputFinder:
    ...
    async def run(self, key_id: CryptoKeyId, input: ComputedDataInput) -> KmsComputedDataResponse:
        crypto_key = await self._cryptokey_repository.find_by_id(key_id)
        res = await self._computed_data_repository.find_one_by_crypto_key_and_input(crypto_key, input)
